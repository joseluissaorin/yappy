...
## Fase 0 · El suelo
- [ ] 0.1 Zoom muerto: viewport (max-scale=1, user-scalable=no, viewport-fit=cover)
      + touch-action: manipulation + user-select none + touch-callout none
      + text-size-adjust none + overscroll-behavior none.
- [ ] 0.2 Háptica NATIVA: Haptics.swift (@_cdecl, generadores precalentados:
...
- [ ] 0.3 Teclado (enterkeyhint, sin autocorrect en URL, esquiva con
      visualViewport), gesto de borde = volver, reduced-motion en todo.

## Fase 1 · El alma: el loro
- [ ] 1.1 Criatura 2.0: marioneta (cabeza, pupilas, párpados, pico con
      apertura 0..1) + estados (posado/hablando/pausa/comiendo/dormido/
      avergonzado/celebrando) + parpadeo y acicalado con temporizador.
- [ ] 1.2 `playback_nivel` (~24 Hz, RMS de la ventana sonando) desde el
      hilo de audio: el sistema nervioso de pico, tipografía y aguja.
- [ ] 1.3 Colocaciones: carrete (mirada sigue el dedo), cameo en el
      cartel, asomado a la boca, comiéndose el modelo, avergonzado en
      errores, dormido de noche.
- [ ] 1.4 Overscroll-loro: cuelga del carrete y silba al refrescar.

## Fase 2 · El cuerpo: cartel
- [ ] 2.1 Peso variable de Literata respirando con el nivel.
- [ ] 2.2 Subrayador-rotulador (borde irregular, grosor por amplitud).
- [ ] 2.3 Frases con {#key}: la vieja se derrama arriba, la nueva aterriza
      con sobreimpulso.
- [ ] 2.4 Swipe de párrafo pegado al dedo con goma.
- [ ] 2.5 Anticipación del mando (inclinación previa).
- [ ] 2.6 Pausa exhala / reanudar inhala; piano escalonado.
- [ ] 2.7 Preguntas inclinadas; títulos con asentimiento.
- [ ] 2.8 Dial-cuentakilómetros con detentes hápticos.

## Fase 3 · La cinta viva
- [ ] 3.1 Cascada al montar. 3.2 Aterrizaje con squash + brinco del loro.
- [ ] 3.3 Descarte que se arranca girando. 3.4 FLIP en recolocaciones.
- [ ] 3.5 Aguja con latido real y 3.6 pegajosa. 3.7 Parallax del riel.
- [ ] 3.8 Boca con muelle (＋→×). 3.9 Bobina girando. 3.10 .pulsado global.

## Fase 4 · El casting
- [ ] La voz elegida = tu loro en toda la app; su tinta se filtra en los
      acentos con color-mix (70 % marca / 30 % voz). Presentación con pico.

## Fase 5 · Completar
- [ ] Audiolibros al idioma nuevo; mini-ondas en el segmento sonando;
      teatro de errores; accesibilidad (VoiceOver sobre gestos, aria es);
      manual al día; Android recompilado; 0.2.0.5 a TestFlight; capturas.
