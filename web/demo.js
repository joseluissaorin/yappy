...
(function () {
  ...
  async function cargarWasm() {
    ...
    const mod = await import((base || './') + 'guion/yappy_wasm.js');
    await mod.default();
  ...
  boton.addEventListener('click', async () => {
    ...
    try {
      ...
      const html = guion.piezas
        ...
        .join('');
      salida.innerHTML = html || '<em>nada que decir</em>';
    } catch (e) {
      console.error(e);
      salida.innerHTML =
        '<span style="color: var(--tinta-suave)">No pude cargar el motor en este navegador. Un ejemplo de lo que hace: «El siglo XIX terminó en 1900» → «El <span class="cambio">siglo diecinueve</span> terminó en <span class="cambio">mil novecientos</span>».</span>';
    } finally {
