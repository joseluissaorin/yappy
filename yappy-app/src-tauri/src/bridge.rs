...
mod desktop_impl {
        ...
        while let Some(msg) = read.next().await {
            ...
            match in_msg {
                InMsg::Hello { token, browser } => {
                    ...
                    if accepted && !token.is_empty() {
                        if let Some(tk) = token_to_store {
                            ...
                            let tk2 = tk.clone();
                            if let Err(e) =
                                crate::settings::update(&app_for_save, &state_for_save, |s| {
                                    s.bridge_token = tk2;
                                })
                            {
                ...
                InMsg::Page {
                    ...
                    let _ = tx.send(serde_json::to_string(&OutMsg::Ack {
                        ok: true,
                        message: None,
                    })?);
                }
                InMsg::Ping => {
                    let _ = tx.send(serde_json::to_string(&OutMsg::Pong)?);
                }
            }
        }
    ...
    pub async fn request_fetch_current_tab(
        ...
        let Some(h) = conns.get(&key) else {
            return false;
        };
        h.tx.send(serde_json::to_string(&OutMsg::FetchCurrentTab).unwrap_or_default())
            .is_ok()
    }
    ...
    fn clean_markdown_for_speech(md: &str) -> String {
        ...
        static DUP_PAREN: Lazy<Regex> = Lazy::new(|| {
            Regex::new(
                r"(\b\p{L}[\p{L}\p{M}0-9'’\-]{1,40})\s*\(\s*([\p{L}\p{M}0-9'’\-]{1,40})\s*\)",
            )
            .unwrap()
        });
