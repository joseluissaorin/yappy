...
async function promover(versionCode, pista = "alpha", estado = "completed") {
  const tok = await token();
  const edit = await api(tok, "/edits", { method: "POST", body: "{}" });
  await api(tok, `/edits/${edit.id}/tracks/${pista}`, {
    method: "PUT",
    body: JSON.stringify({
      releases: [{ versionCodes: [String(versionCode)], status: estado }],
    }),
  });
  const fin = await api(tok, `/edits/${edit.id}:commit`, { method: "POST", body: "{}" });
  console.log(JSON.stringify({ ok: true, pista, versionCode, estado, edit: fin.id }));
}
...
try {
  ...
  else if (cmd === "promote") await promover(args[0], args[1], args[2]);
  else {
    console.error("uso: play-helper.mjs token|status|upload <fichero> [pista]|ficha [raiz] [--sin-imagenes]|listado|promote <versionCode> [pista] [completed|draft]  (app en borrador: draft)");
    process.exitCode = 2;
  }
} catch (e) {
